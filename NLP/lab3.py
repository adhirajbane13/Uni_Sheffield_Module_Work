...
def unigrams_model(structured_sentence_list,Unigrams_pro):
	...
	for candidate_sentence in structured_sentence_list:
		...
		if score_list[0] == score_list[1]: #If the value of two scores are same and both equal to 0, append 2 to represent this.
			answer.append(2)
		elif score_list[0] == 0 or score_list[1] == 0: #If one of the scores of candidate word is 0, append 3 to represent this.
			answer.append(3)
		else:
...
def compute_correct_rate(answer,strandard_answer):
	...
	for i in range(len(answer)):
		if answer[i] == strandard_answer[i]:
			...
			correct_count += 0.5 #(Plus 0.5 because one of the probabilities of candidate word are 0)

...
__device__ float sphere_intersect(const Sphere* s, float ox, float oy, float* n) {
    float dx = ox - s->x;
    float dy = oy - s->y;
...
__device__ float sphere_intersect_read_only(const Sphere* __restrict__ s, float ox, float oy, float* n) {
    float dx = ox - s->x;
    float dy = oy - s->y;
    float radius = s->radius;
    if (dx * dx + dy * dy < radius * radius) {
...
__global__ void ray_trace(uchar4* image, const Sphere* d_s) {
...
__global__ void ray_trace_read_only(uchar4* image, const Sphere* __restrict__ d_s) {
    // map from threadIdx/BlockIdx to pixel position
    int x = threadIdx.x + blockIdx.x * blockDim.x;
    int y = threadIdx.y + blockIdx.y * blockDim.y;
    int offset = x + y * blockDim.x * gridDim.x;
    ...
    for (int i = 0; i < d_sphere_count; i++) {
        const Sphere* s = &d_s[i];
        float n;
        float t = sphere_intersect_read_only(s, ox, oy, &n); // Using read-only cache function
        if (t > maxz) {
            float fscale = n;
            r = s->r * fscale;
            g = s->g * fscale;
...
__global__ void ray_trace_const(uchar4* image) {
    // Kernel using constant data cache
}
...
int main(void) {
    ...
    Sphere* d_s;
    float timing_data[3] = { 0 }; //timing data where [0]=normal, [1]=read-only, [2]=const
    ...
    // generate an image from the sphere data (normal kernel)
    cudaEventRecord(start, 0);
    ray_trace << <blocksPerGrid, threadsPerBlock >> > (d_image, d_s);
    cudaEventRecord(stop, 0);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&timing_data[0], start, stop);
    checkCUDAError("kernel (normal)");

    // generate an image from the sphere data (read-only kernel)
    cudaEventRecord(start, 0);
    ray_trace_read_only << <blocksPerGrid, threadsPerBlock >> > (d_image, d_s);
    cudaEventRecord(stop, 0);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&timing_data[1], start, stop);
    checkCUDAError("kernel (read-only)");

    // generate an image from the sphere data (constant kernel)
    cudaEventRecord(start, 0);
    ray_trace_const << <blocksPerGrid, threadsPerBlock >> > (d_image);
    cudaEventRecord(stop, 0);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&timing_data[2], start, stop);
    ...
    // output timings
    printf("Timing Data Table\n Spheres | Normal | Read-only | Constant\n");
    printf(" %-7i | %-6.3f | %-9.3f | %.3f\n", sphere_count, timing_data[0], timing_data[1], timing_data[2]);
